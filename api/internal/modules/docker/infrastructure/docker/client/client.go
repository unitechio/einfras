...
type ClientFactory struct {
	signatureService     einfra.DigitalSignatureService
	reverseTunnelService einfra.ReverseTunnelService
}

// NewClientFactory returns a new instance of a ClientFactory
func NewClientFactory(signatureService einfra.DigitalSignatureService, reverseTunnelService einfra.ReverseTunnelService) *ClientFactory {
...
func (factory *ClientFactory) CreateClient(endpoint *einfra.Endpoint, nodeName string, timeout *time.Duration) (*client.Client, error) {
	switch endpoint.Type {
	case einfra.AzureEnvironment:
		return nil, errUnsupportedEnvironmentType
	case einfra.AgentOnDockerEnvironment:
		return createAgentClient(endpoint, endpoint.URL, factory.signatureService, nodeName, timeout)
	case einfra.EdgeAgentOnDockerEnvironment:
...
func createLocalClient(endpoint *einfra.Endpoint) (*client.Client, error) {
...
func createTCPClient(endpoint *einfra.Endpoint, timeout *time.Duration) (*client.Client, error) {
...
func createAgentClient(endpoint *einfra.Endpoint, endpointURL string, signatureService einfra.DigitalSignatureService, nodeName string, timeout *time.Duration) (*client.Client, error) {
	httpCli, err := httpClient(endpoint, timeout)
	if err != nil {
		return nil, err
	}

	signature, err := signatureService.CreateSignature(einfra.EInfraAgentSignatureMessage)
	if err != nil {
		return nil, err
	}

	headers := map[string]string{
		einfra.EInfraAgentPublicKeyHeader: signatureService.EncodedPublicKey(),
		einfra.EInfraAgentSignatureHeader: signature,
	}

	if nodeName != "" {
		headers[einfra.EInfraAgentTargetHeader] = nodeName
	}
...
func (t *NodeNameTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if ok {
		for idx, r := range rs {
			...
			nodeNames[fmt.Sprintf("%s-%d", r.ID, idx)] = r.EInfra.Agent.NodeName
		}
	}
...
func httpClient(endpoint *einfra.Endpoint, timeout *time.Duration) (*http.Client, error) {
