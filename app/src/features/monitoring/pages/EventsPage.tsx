...
export default function EventsPage() {
  ...
  const runtimeAuditQuery = useRuntimeAudit(
    selectedEnvironment?.type === "docker" ? selectedEnvironment.id : "",
  );
  const [platformView] = useState<"platform" | "docker">(
    selectedEnvironment?.type === "docker" ? "docker" : "platform",
  );
  ...
  const getEventIcon = (action: string) => {
    ...
    if (a.includes("delete") || a.includes("remove") || a.includes("kill"))
      return Trash2;
    if (a.includes("create") || a.includes("add")) return PlusCircle;
    if (a.includes("restart") || a.includes("update")) return RefreshCw;
    return Activity;
  };
    ...
    if (
      ...
      return (runtimeAuditQuery.data ?? []).map((event) => {
        ...
        return {
          ...
          description:
            event.details || `${event.resource_type} ${event.resource_id}`,
          time: new Date(event.created_at).toLocaleString(),
          severity:
            event.status === "failed"
              ? ("danger" as const)
              : event.action.includes("delete") || event.action.includes("kill")
                ? ("warning" as const)
                : ("info" as const),
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            size="sm"
            className="h-9"
            onClick={() => runtimeAuditQuery.refetch()}
          >
            <RefreshCw
              className={cn(
                "mr-2 h-3.5 w-3.5",
                runtimeAuditQuery.isFetching && "animate-spin",
              )}
            />
            Refresh
          </Button>
          ...
          {filteredEvents.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-20 text-zinc-500 dark:text-zinc-400">
              ...
              <p className="text-sm font-medium">
                No events captured in this timeline.
              </p>
            </div>
          ) : (
            filteredEvents.map((event) => {
              ...
              return (
                <div
                  key={event.id}
                  className="group border-l-2 border-transparent transition-all hover:border-indigo-500 hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30"
                >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className="px-1.5 text-zinc-300 dark:text-zinc-700">
                            /
                          </span>
                          <Badge
                            variant="outline"
                            className="h-5 rounded-md px-2 py-0 text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400"
                          >
                            {event.user}
                          </Badge>
                          {event.ip && (
                            <Badge
                              variant="outline"
                              className="h-5 flex items-center gap-1 rounded-md px-2 py-0 text-[10px] font-bold text-zinc-500 dark:text-zinc-400"
                            >
