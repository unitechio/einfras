...
export default function UserProfilePage() {
    ...
    void settingsApi
      ...
      .then((response) => {
        ...
        setSecurityForm((current) => ({
          ...
          sessionTimeout: readString(
            security.sessionTimeout,
            current.sessionTimeout,
          ),
          requireMFA: readBoolean(security.requireMFA, current.requireMFA),
          notifyNewLogin: readBoolean(
            security.notifyNewLogin,
            current.notifyNewLogin,
          ),
        }));
  ...
  const initials = useMemo(() => {
    const source =
      profileForm.fullName.trim() || session?.user.username || "AD";
    return (
      source
        .split(/\s+/)
        .slice(0, 2)
  ...
  async function persist(
    ...
    try {
      ...
      showNotification({
        ...
        description:
          "Profile settings are now reflected across your workspace.",
      });
    } catch (error) {
      showNotification({
        type: "error",
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="relative overflow-hidden rounded-md border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-[#121212]">
        ...
        <div className="flex flex-col items-center gap-6 md:flex-row">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="mb-1 flex flex-col gap-2 md:flex-row md:items-center">
              ...
              <Badge
                variant="outline"
                className="mx-auto w-fit border-indigo-100 bg-indigo-50 px-2 text-[10px] font-semibold uppercase tracking-widest text-indigo-600 dark:mx-0 dark:border-indigo-500/20 dark:bg-indigo-500/10 dark:text-indigo-400"
              >
          ...
          <div className="shrink-0">
            <Button
              variant="outline"
              size="md"
              className="group transition-all hover:border-red-300 hover:text-red-500 dark:hover:border-red-500/30"
            >
              <LogOut
                size={14}
      ...
      <div className="flex w-fit items-center gap-1 rounded-md border border-zinc-200 bg-zinc-100 p-1 dark:border-zinc-800 dark:bg-zinc-900/50">
        {tabs.map((tab) => (
          <button
            key={tab.id}
      ...
      <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          {isLoading ? (
            <div className="rounded-md border border-zinc-200 bg-white p-6 text-sm text-zinc-500 shadow-sm dark:border-zinc-800 dark:bg-[#121212]">
              Loading profile settings...
            </div>
          ) : null}

          {!isLoading && activeTab === "profile" ? (
            <div className="space-y-6 rounded-md border border-zinc-200 bg-white p-6 shadow-sm animate-in fade-in slide-in-from-left-2 duration-300 dark:border-zinc-800 dark:bg-[#121212]">
              <div>
                <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">
                  Personal Details
                </h3>
                <p className="mt-0.5 text-xs text-zinc-600 dark:text-zinc-400">
                  Update your identity information and keep the profile header
                  in sync.
                </p>
              </div>
          ...
          {!isLoading && activeTab === "security" ? (
            <div className="space-y-6 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm animate-in fade-in slide-in-from-left-2 duration-300 dark:border-zinc-800 dark:bg-[#121212]">
              <div>
                ...
                <p className="mt-0.5 text-xs text-zinc-600 dark:text-zinc-400">
                  Persist your security operations defaults so the page reflects
              ...
              <div className="space-y-3">
                ...
                <SecurityCard
                  ...
                  description={
                    securityForm.requireMFA
                      ? "MFA is required for this profile."
                      : "MFA requirement is currently relaxed for this profile."
                  }
                  action={
                    <button
                      type="button"
              ...
              <label className="flex items-center justify-between rounded-xl border border-zinc-200 px-4 py-3 text-sm dark:border-zinc-800">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setSecurityForm((current) => ({
                      ...current,
                      notifyNewLogin: event.target.checked,
                    }))
                  }
                ...
                <div className="flex items-center justify-between rounded-lg border border-zinc-200 bg-zinc-50 p-3 font-mono text-xs text-zinc-600 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-400">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      showNotification({
                        type: "info",
                        message: "API key manager",
                        description:
                          "Dedicated key lifecycle management will be wired here next.",
                      })
                    }
                  >
                    Manage
              ...
              <div className="flex justify-end border-t border-zinc-100 pt-4 dark:border-zinc-800">
                <Button
                  variant="primary"
                  onClick={() => void saveSecuritySection()}
                  isLoading={isSaving === "security"}
                >
          ...
          {!isLoading && activeTab === "preferences" ? (
            <div className="space-y-6 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm animate-in fade-in slide-in-from-left-2 duration-300 dark:border-zinc-800 dark:bg-[#121212]">
              <div>
                ...
                <p className="mt-0.5 text-xs text-zinc-600 dark:text-zinc-400">
                  Theme, language, density, and start page are now loaded from
                  saved user settings.
                </p>
              </div>
              ...
              <div className="flex justify-end border-t border-zinc-100 pt-4 dark:border-zinc-800">
                <Button
                  variant="primary"
                  onClick={() => void savePreferencesSection()}
                  isLoading={isSaving === "preferences"}
                >
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-[#121212]">
            <h4 className="mb-5 text-xs font-semibold uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
              Usage & Limits
            </h4>
            ...
            <Button
              variant="outline"
              size="md"
              className="group mt-6 w-full border-dashed text-xs text-zinc-500 transition-all hover:border-indigo-300 hover:text-indigo-500 dark:hover:border-indigo-500/30"
            >
              Upgrade Plan{" "}
              <ChevronRight
                size={12}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 dark:text-zinc-400">
                  ...
                  security audits for production servers.
                </p>
