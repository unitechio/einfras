...
export default function FeatureFlagsSettingsPage() {
  ...
  const categories = useMemo(
    () => [
      "all",
      ...Object.keys(groupedFlags).sort((left, right) =>
  ...
  const groupedFilteredFlags = useMemo(() => {
    return filteredFlags.reduce<Record<string, RuntimeFeatureFlag[]>>(
      (acc, item) => {
        const key = item.category || "general";
        acc[key] = [...(acc[key] ?? []), item];
        return acc;
  ...
  const saveFlags = async () => {
    ...
    try {
      ...
      showNotification({
        ...
        description:
          "Runtime gates and settings screens now use the latest persisted state.",
      });
    } catch (err) {
      showNotification({
        type: "error",
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Control runtime capabilities by category, search across flags, and
            save a cleaner feature posture.
          </p>
        </div>
        <Button
          variant="primary"
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-md border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-[#121212]">
      ...
      <div className="flex flex-col gap-3 rounded-md border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-[#121212] lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="space-y-5">
        {Object.entries(groupedFilteredFlags)
          ...
          .map(([group, items]) => (
            <section
              key={group}
              className="rounded-md border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-[#121212]"
            >
              <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <p className="text-sm text-zinc-500">
                    {items.length} flags in this capability group.
                  </p>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => toggleCategory(group, true)}
                  >
                    Enable All
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => toggleCategory(group, false)}
                  >
                    Disable All
                  </Button>
                </div>
              ...
              <div className="space-y-3">
                {items.map((flag) => (
                  ...
                  >
                    ...
                    >
                      <span
                        className={`mt-1 block h-4 w-4 rounded-full bg-white transition-transform ${flag.enabled ? "translate-x-6" : "translate-x-1"}`}
                      />
                    </button>
                  </div>
                ))}
