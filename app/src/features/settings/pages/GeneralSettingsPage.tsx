...
export default function GeneralSettingsPage() {
  ...
  const handleSave = (section: string) => {
    showNotification({
      ...
      description: `Your changes to ${section.toLowerCase()} have been applied successfully.`,
    });
  };
  ...
  const handleDownloadBackup = () => {
    if (passwordProtect && !backupPassword) {
      showNotification({
        ...
        description: "Please enter a backup password.",
      });
      return;
    }
    showNotification({
    ...
    <div className="bg-white dark:bg-[#121212] border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm overflow-hidden mb-6 filter drop-shadow-sm">
      <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/10 flex items-center gap-3">
        ...
        {badge && (
          <Badge
            variant="outline"
            className="ml-auto flex items-center gap-1.5 bg-yellow-50 text-yellow-700 border-yellow-200 dark:bg-yellow-500/10 dark:text-yellow-500 dark:border-yellow-500/20"
          >
            <Briefcase size={12} />
            {badge}
          </Badge>
      ...
      <div className="flex-1 pr-6">
        <label className="text-sm font-medium text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
          ...
          {businessFeature && (
            <Badge
              variant="outline"
              className="text-[10px] px-1.5 py-0 h-4 bg-indigo-50 text-indigo-600 border-indigo-200 dark:bg-indigo-500/10 dark:text-indigo-400 dark:border-indigo-500/20"
            >
              Pro
            </Badge>
          )}
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
            Manage global configuration for your instance.
          </p>
        </div>
      </div>
      ...
      <Section title="Application Settings" icon={Globe}>
        <div className="space-y-2">
          <SettingRow
            ...
            description="How often environments are polled for status updates."
          >
            <Input
          ...
          <SettingRow
            label="Edge agent poll frequency"
            description="Default interval for edge agents to phone home."
          >
            <select
              value={pollFrequency}
              onChange={(e) => setPollFrequency(e.target.value)}
              className="w-full h-10 px-3 py-2 bg-white dark:bg-[#121212] border border-zinc-200 dark:border-zinc-800 rounded-md text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-shadow"
            >
          ...
          <SettingRow
            label="Custom Logo"
            description="Replace the default brand logo across the application."
            businessFeature
          >
            <div className="flex items-center justify-end">
              <Switch
                checked={useCustomLogo}
                onChange={() => setUseCustomLogo(!useCustomLogo)}
              />
            </div>
          </SettingRow>
          <SettingRow
          ...
          >
            <div className="flex items-center justify-end">
              <Switch
                checked={allowAnonymousStats}
                onChange={() => setAllowAnonymousStats(!allowAnonymousStats)}
              />
            </div>
          </SettingRow>
          <SettingRow
            label="Login Screen Banner"
            description="Display an informational message on the login screen."
            businessFeature
          >
            <div className="flex items-center justify-end">
              <Switch
                checked={loginBanner}
                onChange={() => setLoginBanner(!loginBanner)}
              />
            </div>
          </SettingRow>
          <SettingRow
            label="App Templates URL"
            description="Provide a custom registry JSON for one-click templates."
          >
            <Input
        ...
        <div className="mt-8 flex justify-end">
          <Button
            variant="primary"
            onClick={() => handleSave("Application Settings")}
          >
      ...
      <Section title="Kubernetes Configuration" icon={Database}>
        <div className="space-y-2">
          <SettingRow
            label="Global Helm Repository"
            description="Default repository for deploying charts."
          >
            <Input
              value={helmRepoUrl}
          ...
          <SettingRow
            label="Kubeconfig Expiry"
            description="Maximum lifetime for user-downloaded kubeconfig files."
          >
            <select
          ...
          <SettingRow
            label="Enforce GitOps"
            description="Prevent UI-based deployments, demanding code-based configs."
            businessFeature
          >
            <div className="flex items-center justify-end">
              <Switch
                checked={enforceCodeDeployment}
                onChange={() =>
          ...
          <SettingRow
            label="Require Change Notes"
            description="Mandate a reason when applications are updated."
            businessFeature
          >
            <div className="flex items-center justify-end">
              <Switch
          ...
          <SettingRow
            label="Enable K8s Stacks"
            description="Allow stack functionality via Custom Resource Definitions."
          >
            <div className="flex items-center justify-end">
              <Switch
                checked={allowStacks}
                onChange={() => setAllowStacks(!allowStacks)}
              />
            </div>
          </SettingRow>
        </div>
        <div className="mt-8 flex justify-end">
          <Button
            variant="primary"
            onClick={() => handleSave("Kubernetes Settings")}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <Section title="Helm CA Certificates" icon={Key} badge="Business">
          <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-6 leading-relaxed">
            Provide additional Certificate Authority (CA) files for establishing
            trusted TLS connections to private Helm repositories.
          </p>
          <div className="space-y-4">
            <div className="border border-dashed border-zinc-300 dark:border-zinc-700 rounded-lg p-6 flex flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-900/20 hover:bg-zinc-100 dark:hover:bg-zinc-800/40 transition-colors cursor-pointer group">
              ...
              <p className="text-xs text-zinc-500 mt-1">
                .pem, .crt, or .cer extensions supported
              </p>
            </div>
          </div>
          <div className="mt-6 flex justify-end">
            <Button
              variant="outline"
              onClick={() => handleSave("CA Certificates")}
            >
              Upload & Apply
            </Button>
          </div>
        ...
        <Section title="SSL Certificates" icon={Lock}>
          <div className="flex items-start gap-3 p-3.5 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-lg mb-6">
            ...
            <p className="text-xs text-amber-800 dark:text-amber-300 leading-relaxed">
              Forcing HTTPS will instantly close HTTP ports. Edge agents relying
              on HTTP connections will be disconnected entirely.
            </p>
          </div>
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <Switch
                checked={forceHttps}
                onChange={() => setForceHttps(!forceHttps)}
              />
            </div>
          ...
          <div className="mt-6 flex justify-end">
            <Button
              variant="primary"
              onClick={() => handleSave("SSL Settings")}
            >
              Save SSL Config
            </Button>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="flex-1 space-y-4">
            <p className="text-sm text-zinc-600 dark:text-zinc-400">
              Specify label keys/values to globally hide specific containers
              from lists in the UI.
            </p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">
                  Label Key <span className="text-red-500">*</span>
                </label>
                <Input
                  placeholder="com.example.system"
                  value={filterName}
                ...
                <TableBody>
                  {filters.length === 0 ? (
                    <TableRow>
                      <TableCell
                        colSpan={3}
                        className="text-center py-6 text-zinc-500 dark:text-zinc-400 text-xs"
                      >
                        No filters defined.
                      </TableCell>
                    </TableRow>
                      ...
                      <TableRow key={i}>
                        ...
                        <TableCell className="py-2 text-right">
                          <button
                            ...
                            className="text-zinc-400 hover:text-red-500 transition-colors text-xs font-medium"
                          >
                            Remove
                          </button>
                        </TableCell>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_350px] gap-8">
          <div>
            ...
            <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-6">
              Generate a complete archive of your system configuration,
              environments, and RBAC rules. Note: Container volumes and images
              are not included.
            </p>
            ...
            <div className="grid grid-cols-2 gap-4 max-w-xl">
              <div
                ...
                className={cn(
                  "border rounded-xl p-5 cursor-pointer transition-all flex flex-col",
                  backupOption === "download"
                    ? "border-indigo-500 bg-indigo-50/50 dark:bg-indigo-500/10 ring-1 ring-indigo-500"
                    : "border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 bg-white dark:bg-[#121212]",
                )}
              >
