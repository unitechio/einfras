...
import { Copy, KeyRound, Plus, RefreshCw, Save, Shield, Sparkles, Trash2 } from "lucide-react";
import { useNotification } from "@/core/NotificationContext";
import { Button } from "@/shared/ui/Button";
import { Input } from "@/shared/ui/Input";
import { Badge } from "@/shared/ui/Badge";
import { settingsApi, type RuntimeLicense, type RuntimeLicenseKey } from "@/features/settings/api";
...
const emptyKeyDraft = (): Partial<RuntimeLicenseKey> => ({
  ...
  issued_to: "",
  expires_at: null,
  features: ["runtime_topology", "docker_compose_runtime", "integration_providers"],
  metadata: {},
});
...
export default function LicenseSettingsPage() {
  ...
  const [draft, setDraft] = useState<Partial<RuntimeLicenseKey>>(emptyKeyDraft());
  ...
  const generateLicense = async () => {
    ...
    try {
      ...
      await loadAll();
      showNotification({ type: "success", message: "Primary license key regenerated" });
    } catch (err) {
      showNotification({
        type: "error",
  ...
  const saveLicenseKey = async () => {
    ...
    try {
      const payload = {
        ...draft,
        expires_at: draft.expires_at ? `${String(draft.expires_at).slice(0, 10)}T00:00:00Z` : "",
      };
  ...
  const copyKey = async (value?: string) => {
    ...
    try {
      ...
      showNotification({ type: "error", message: "Unable to copy license key" });
    }
  };
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Manage the active organization license plus a reusable inventory of customer-facing activation keys.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={() => void generateLicense()} isLoading={isGenerating}>
            <RefreshCw className="mr-2 h-4 w-4" />
            Regenerate Active Key
          </Button>
          <Button variant="primary" onClick={() => void saveLicenseProfile()} isLoading={isSaving}>
        ...
        <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-[#121212]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Active organization license</h2>
              <p className="mt-1 text-sm text-zinc-500">This is the currently promoted license used by runtime gating and entitlement summaries.</p>
            </div>
            <Button variant="outline" onClick={() => void copyKey(license.license_key)}>
              <Copy className="mr-2 h-4 w-4" />
              Copy Active Key
            </Button>
          </div>
          <div className="mt-4 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-4 font-mono text-sm tracking-[0.18em] text-zinc-900 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100">
            {license.license_key || "No active key"}
          </div>
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <Input value={license.tier} onChange={(e) => setLicense({ ...license, tier: e.target.value })} placeholder="Tier" />
            <Input value={license.status} onChange={(e) => setLicense({ ...license, status: e.target.value })} placeholder="Status" />
            <Input value={license.contact_email} onChange={(e) => setLicense({ ...license, contact_email: e.target.value })} placeholder="Contact email" />
            <Input value={license.expires_at ? license.expires_at.slice(0, 10) : ""} onChange={(e) => setLicense({ ...license, expires_at: e.target.value ? `${e.target.value}T00:00:00Z` : null })} placeholder="Expiry date" />
          </div>
        </section>
      ) : null}
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        <div className="space-y-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-[#121212]">
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Key Inventory</div>
          ...
          {licenseKeys.map((key) => (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <div className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">{key.name}</div>
                <Badge variant={key.is_primary ? "success" : "outline"}>{key.is_primary ? "primary" : key.status}</Badge>
              </div>
              <div className="mt-1 text-xs text-zinc-500">{key.tier}</div>
              <div className="mt-3 font-mono text-[11px] text-zinc-500">{key.license_key}</div>
            </button>
          ))}
        </div>

        <div className="space-y-6 rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-[#121212]">
          <div className="grid gap-4 md:grid-cols-2">
            <Input value={draft.name ?? ""} onChange={(e) => updateDraft({ name: e.target.value })} placeholder="Customer Production Key" />
            <Input value={draft.issued_to ?? ""} onChange={(e) => updateDraft({ issued_to: e.target.value })} placeholder="Issued to / account name" />
            <Input value={draft.tier ?? "free"} onChange={(e) => updateDraft({ tier: e.target.value })} placeholder="Tier" />
            <Input value={draft.status ?? "draft"} onChange={(e) => updateDraft({ status: e.target.value })} placeholder="Status" />
            <Input value={draft.expires_at ? String(draft.expires_at).slice(0, 10) : ""} onChange={(e) => updateDraft({ expires_at: e.target.value || null })} placeholder="Expiry date" />
            <label className="flex items-center justify-between rounded-xl border border-zinc-200 px-4 py-3 text-sm dark:border-zinc-800">
              ...
              <input type="checkbox" checked={draft.is_primary ?? false} onChange={(e) => updateDraft({ is_primary: e.target.checked })} />
            </label>
          </div>
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-[#121212]">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-zinc-500">Control which features this specific license key can activate.</p>
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-[#121212]">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-zinc-500">Persist or retire customer-facing keys without losing the main organization license profile.</p>
                </div>
              </div>
              <div className="space-y-3">
                {selectedKey?.license_key ? (
                  <Button variant="outline" className="w-full" onClick={() => void copyKey(selectedKey.license_key)}>
                    <Copy className="mr-2 h-4 w-4" />
                    Copy Selected Key
                  </Button>
                ) : null}
                {selectedKey ? (
                  <Button variant="outline" className="w-full text-red-600" onClick={() => void deleteLicenseKey()} isLoading={isSaving}>
                    <Trash2 className="mr-2 h-4 w-4" />
                    Delete Selected Key
                  </Button>
                ) : null}
                <Button variant="primary" className="w-full" onClick={() => void saveLicenseKey()} isLoading={isSaving}>
                  <Save className="mr-2 h-4 w-4" />
                  Save License Key
                </Button>
              </div>
