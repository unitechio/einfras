...
import {
  ...
  useStacks,
  useVolumes,
} from "@/features/docker/api/useDockerHooks";
...
import {
  ...
  usePods,
} from "@/features/kubernetes/api/useKubernetesHooks";
...
function DockerDashboard({ environmentId }: { environmentId: string }) {
  ...
  const capacityHistoryQuery = useQuery({
    ...
    queryFn: () =>
      environment?.serverId
        ? monitoringApi.getMetricsHistory(environment.serverId, 12)
        : Promise.resolve([]),
  ...
  const dockerNodes = useMemo(() => {
    return nodeQueries
      ...
      .filter(
        (item) =>
          item.agent?.has_docker ||
          item.server.provider === "local-control-plane",
      )
      .map((item) => ({
        id: item.server.id,
        ...
        memory: Number(
          item.agent?.mem_percent ?? item.metrics?.memPercent ?? 0,
        ),
  ...
  const runningContainers = (containersQuery.data ?? []).filter(
  ...
  const topContainerStatQueries = useQueries({
    queries: runningContainers.slice(0, 5).map((container) => ({
      ...
      queryFn: () =>
        apiFetch(
          `/v1/environments/${environmentId}/docker/containers/${container.Id}/stats`,
        ).catch(() => null),
  ...
  const containerStateDistribution = buildDistribution(
    containers.map((item) => normalizeContainerState(item.State)),
  );
  const containerStateChart = distributionToChart(containerStateDistribution);
  const containerCapacityTrend = buildCapacityTrend(
    [capacityHistoryQuery.data ?? []],
    Number(environment.cpuPercent ?? 0),
    Number(environment.memPercent ?? 0),
  );
  const topContainerUsage = runningContainers
    .slice(0, 5)
    .map((container, index) => {
      const stats = topContainerStatQueries[index]?.data as
        | { cpu_perc?: string; mem_perc?: string }
        | null
        | undefined;
      return {
  ...
  const usedStoragePercent = Math.max(
    0,
    Math.min(100, Number(environment.diskPercent ?? 0)),
  );
  ...
  const infoCards = [
    ...
    {
      label: "Running vs Stopped",
      value: `${runningContainers.length} / ${containers.length - runningContainers.length}`,
      icon: Monitor,
      tone: "emerald",
    },
    {
      label: "Images / Volumes",
      value: `${images.length} / ${volumes.length}`,
      icon: HardDrive,
      tone: "amber",
    },
    {
      label: "Networks / Stacks",
      value: `${networks.length} / ${stacks.length}`,
      icon: Share2,
      tone: "indigo",
    },
  ] as const;
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Host health, Docker daemon details, node inventory, and runtime
      ...
      <div className="grid gap-6 xl:grid-cols-2">
          ...
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Running, exited, paused, and unhealthy containers at a glance.
              </p>
            </div>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">
                Cluster CPU / Memory Trend
              </h2>
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Recent host pressure behind this Docker environment.
              </p>
            </div>
            <Badge variant="outline">
              {environment.stats?.serverVersion ?? "docker"}
            </Badge>
          </div>
          <TrendAreaChart
            data={containerCapacityTrend}
            firstKey="cpu"
            secondKey="memory"
            firstColor="#2563eb"
            secondColor="#0f766e"
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                The host information Portainer usually highlights in its
                dashboard.
              </p>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <InfoTile
              label="Context"
              value={environment.stats?.currentContext || "default"}
            />
            <InfoTile
              label="Storage Driver"
              value={environment.stats?.storageDriver || "unknown"}
            />
            <InfoTile
              label="Docker Root"
              value={environment.stats?.dockerRootDir || "unknown"}
            />
            <InfoTile
              label="Kernel / OS"
              value={
                [
                  environment.stats?.kernelVersion,
                  environment.stats?.operatingSystem || environment.os,
                ]
                  .filter(Boolean)
                  .join(" • ") || "unknown"
              }
            ...
            <InfoTile
              label="Capacity"
              value={`${environment.cpuCores ?? 0} CPU • ${Number(environment.memoryGB ?? 0).toFixed(1)} GB RAM • ${environment.diskGB ?? 0} GB disk`}
            />
          </div>
          <div className="mt-6 space-y-3">
            ...
            <UsageBar
              label="Disk"
              value={Number(environment.diskPercent ?? 0)}
            />
          </div>
        </Card>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Core workload objects and lifecycle entry points.
              </p>
            </div>
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <Card className="p-6 xl:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Top running containers by live CPU and memory pressure.
              </p>
            </div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <MetricChip
              label="Restart Risk"
              value={
                containers.filter((item) => /restarting/i.test(item.Status))
                  .length
              }
            />
            ...
            <MetricChip
              ...
              value={
                images.filter(
                  (item) =>
                    !item.RepoTags?.length ||
                    item.RepoTags.includes("<none>:<none>"),
                ).length
              }
            />
          </div>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-zinc-500">
                License and alerts at a glance.
              </div>
            </div>
          </div>
          <div className="space-y-3">
            <InfoTile
              label="License Tier"
              value={licenseQuery.data?.tier ?? "unknown"}
            />
            <InfoTile
              label="License Status"
              value={licenseQuery.data?.status ?? "unknown"}
            />
            <InfoTile
            ...
            <InfoTile
              ...
              value={String(applicationsQuery.data?.length ?? 0)}
            />
          </div>
        </Card>
      </div>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Used vs idle containers and disk posture.
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 space-y-3">
            ...
            <UsageBar
              ...
              value={Number(environment.memPercent ?? 0)}
            />
          </div>
        </Card>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Largest images and storage attachments.
              </p>
            </div>
          </div>
          <div className="space-y-3">
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Container and network connectivity density.
              </p>
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
      ...
      <Card className="p-6">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Online status, labels, and resource usage across Docker-capable
              nodes.
            </p>
          </div>
      ...
      <Card className="p-6">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Recent runtime actions, deploys, and operational anomalies for
...
function KubernetesOverview({ environmentId }: { environmentId: string }) {
  ...
  const deploymentsQuery = useDeployments(environmentId, "default", {
    watch: true,
  });
  ...
  const podStatusDistribution = buildDistribution(
    pods.map((item) => normalizePodStatus(item.status)),
  );
  ...
  const clusterHealthScore = calculateClusterHealthScore({
    ...
    healthyDeployments: deployments.filter(
      (item) => item.ready_replicas >= item.desired_replicas,
    ).length,
    totalDeployments: deployments.length || 1,
  });
  const clusterCapacityTrend = buildCapacityTrend(
    [clusterHistoryQuery.data ?? []],
    Number(environment.cpuPercent ?? 0),
    Number(environment.memPercent ?? 0),
  );
  const deploymentMismatch = deployments.filter(
    (item) => item.ready_replicas !== item.desired_replicas,
  );
  const totalRestarts = pods.reduce(
    (total, item) => total + Number(item.restarts ?? 0),
    0,
  );
  const namespaceWorkload = namespaces.slice(0, 6).map((namespace) => ({
    name: namespace.name,
    primary: pods.filter((pod) => pod.namespace === namespace.name).length,
    secondary: deployments.filter(
      (deployment) => deployment.namespace === namespace.name,
    ).length,
    tertiary: storage.filter((volume) =>
      volume.claim.startsWith(`${namespace.name}/`),
    ).length,
  }));
  ...
  const selectedNodePods = pods.filter(
    (item) => item.node === selectedNodeName,
  );
  const selectedNodePressure = buildNodePressure(
    nodes.find((item) => item.name === selectedNodeName),
  );
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="primary" onClick={() => navigate("/deployments")}>
            Deployments
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricCardSimple
          label="Pods Running"
          value={podStatusDistribution.running ?? 0}
          icon={Layers}
        />
        <MetricCardSimple
          label="Namespaces"
          value={namespaces.length || (environment.stats?.namespaces ?? 0)}
          icon={Database}
        />
        <MetricCardSimple
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-zinc-500 dark:text-zinc-400">
                Pod distribution and whole-cluster usage trends.
              </div>
            </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-3">
              <UsageBar
                label="CPU Usage"
                value={Number(environment.cpuPercent ?? 0)}
              />
              <UsageBar
                label="Memory Usage"
                value={Number(environment.memPercent ?? 0)}
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-zinc-500 dark:text-zinc-400">
                Ready state, node pressure, and workload placement.
              </div>
            </div>
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      {selectedNodeName === node.name ? (
                        <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-blue-600 dark:text-blue-300">
                          Focused
                        </span>
                      ) : null}
                    </div>
                    <Badge
                      variant={node.status === "Ready" ? "success" : "warning"}
                    >
                      {node.status}
                    </Badge>
                  </div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            ...
            <MetricChip
              ...
              value={selectedNodePressure.memoryPressure ? 1 : 0}
            />
            <MetricChip
              label="Disk Pressure"
              value={selectedNodePressure.diskPressure ? 1 : 0}
            />
          </div>
        </Card>
      </div>
        ...
        <Card className="p-6 xl:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Replica drift, restart pressure, and tracked applications.
              </p>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <MetricChip
              ...
              value={deploymentMismatch.length}
            />
            <MetricChip label="Restart Count" value={totalRestarts} />
            <MetricChip
              label="Failed Pods"
              value={podStatusDistribution.failed ?? 0}
            />
          </div>
          <div className="mt-5 space-y-3">
            {deploymentMismatch.slice(0, 5).map((deployment) => (
              ...
              >
                ...
                <div className="mt-1 text-xs text-zinc-500">
                  {deployment.namespace} • {deployment.status}
                </div>
              </div>
            ))}
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Open issues and unread alerts across the workspace.
              </p>
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Persistent volumes and storage usage posture.
              </p>
            </div>
          </div>
          <div className="space-y-3">
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Pod and deployment spread by namespace.
              </p>
            </div>
          </div>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Cluster-facing notifications and drift signals.
              </p>
            </div>
          </div>
          <div className="space-y-3">
            {(notificationsQuery.data ?? []).slice(0, 5).map((item) => (
              ...
              >
                ...
                <div className="mt-1 text-xs text-zinc-500">
                  {item.channel} • {formatRelativeTimestamp(item.created_at)}
                </div>
              </div>
            ))}
          </div>
...
function AdminOverview() {
  ...
  const kubernetesEnvironments = inventory.filter(
    (item) => item.type === "kubernetes",
  );
  ...
  const issueApplications = applications.filter(
    (item) => item.status.toLowerCase() !== "healthy",
  );
  ...
  const averageCpu = inventory.length
    ? Math.round(
        inventory.reduce(
          (total, item) => total + Number(item.cpuPercent ?? 0),
          0,
        ) / inventory.length,
      )
    : 0;
  const averageMemory = inventory.length
    ? Math.round(
        inventory.reduce(
          (total, item) => total + Number(item.memPercent ?? 0),
          0,
        ) / inventory.length,
      )
    : 0;
  const totalCpuCores = servers.reduce(
    (total, item) => total + Number(item.cpu_cores ?? 0),
    0,
  );
  const totalMemoryGb = servers.reduce(
    (total, item) => total + Number(item.memory_gb ?? 0),
    0,
  );
  const totalDiskGb = servers.reduce(
    (total, item) => total + Number(item.disk_gb ?? 0),
    0,
  );
  ...
  const topCapacityServers = [...onlineServers]
    .sort(
      (left, right) =>
        Number(right.cpu_cores ?? 0) - Number(left.cpu_cores ?? 0),
    )
    .slice(0, 8);
  ...
  const capacityHistoryQueries = useQueries({
    queries: topCapacityServers.map((server) => ({
      ...
      queryFn: () =>
        monitoringApi.getMetricsHistory(server.id, 12).catch(() => []),
      staleTime: 30_000,
      refetchInterval: 30_000,
    })),
  });
  ...
  const topDockerEnvironments = useMemo(
    () =>
      [...dockerEnvironments]
        .sort(
          (left, right) =>
            scoreDockerEnvironment(right) - scoreDockerEnvironment(left),
        )
        .slice(0, 5),
    [dockerEnvironments],
  );
  const topKubernetesEnvironments = useMemo(
    () =>
      [...kubernetesEnvironments]
        .sort(
          (left, right) =>
            scoreKubernetesEnvironment(right) -
            scoreKubernetesEnvironment(left),
        )
        .slice(0, 5),
    [kubernetesEnvironments],
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
        ...
        <div className="grid gap-8 xl:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm text-zinc-500 dark:text-zinc-400">
                  A control-plane dashboard for environments, tracked
                  applications, notifications, entitlements, and feature
                  posture.
                </p>
              </div>
            </div>
          ...
          <div className="rounded-3xl border border-zinc-200 bg-zinc-50/70 p-6 dark:border-zinc-800 dark:bg-zinc-950/30">
            ...
            <div className="mt-3 text-2xl font-semibold text-zinc-900 dark:text-zinc-50">
              {licenseQuery.data?.tier?.toUpperCase() ?? "UNKNOWN"}
            </div>
            <div className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
              {licenseQuery.data?.status ?? "unknown"}
            </div>
            <div className="mt-4 grid grid-cols-2 gap-3">
              <MetricChip
                label="Enabled Flags"
                value={flags.filter((item) => item.enabled).length}
              />
              <MetricChip
                label="Critical Alerts"
                value={criticalNotifications.length}
              />
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-8">
        ...
        <MetricCardSimple
          label="Docker Env"
          value={dockerEnvironments.length}
          icon={Box}
        />
        <MetricCardSimple
          label="K8s Env"
          value={kubernetesEnvironments.length}
          icon={Globe}
        />
        <MetricCardSimple
          label="Online Users (5m)"
          value={presence?.online_users_5m ?? 0}
          icon={Activity}
        />
        <MetricCardSimple
          label="Online Users (15m)"
          value={presence?.online_users_15m ?? 0}
          icon={Activity}
        />
        <MetricCardSimple
          label="Active Sessions"
          value={presence?.active_sessions ?? 0}
          icon={Monitor}
        />
        <MetricCardSimple
          label="Admins"
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Seven-day alert posture across open and critical notifications.
              </p>
            </div>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">
                Fleet Capacity Trend
              </h2>
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Recent CPU and memory history sampled from the strongest online
                nodes in the fleet.
              </p>
            </div>
            <Badge variant="outline">
              {topCapacityServers.length} nodes sampled
            </Badge>
          </div>
          <TrendAreaChart
            data={capacityTrend}
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Switching into an environment turns the dashboard into Docker or
            ...
            {(inventoryQuery.data ?? []).slice(0, 5).map((environment) => (
              ...
              >
                <div>
                  ...
                  <div className="mt-1 text-xs text-zinc-500">
                    {environment.type} • {environment.url}
                  </div>
                </div>
                <Badge
                  variant={environment.status === "up" ? "success" : "outline"}
                >
                  {environment.status}
                </Badge>
              </div>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Recent notifications and routing-sensitive operational events.
              </p>
            </div>
            <Button
              variant="outline"
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Total estate footprint and average control-plane utilization.
              </p>
            </div>
          </div>
          <div className="space-y-3">
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Application health, unread alerts, and environment availability.
              </p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <MetricChip
              label="Healthy Apps"
              value={healthyApplications.length}
            />
          ...
          <div className="mt-4 rounded-xl border border-zinc-200 px-4 py-3 text-sm text-zinc-600 dark:border-zinc-800 dark:text-zinc-300">
            {teams.length} teams and {roles.length} roles are configured.{" "}
            {activeUsers.length} enabled users exist, while{" "}
            {presence?.online_users_5m ?? 0} users are currently active in the
            last 5 minutes.
          </div>
        </Card>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                How the managed estate is distributed across runtime types.
              </p>
            </div>
          </div>
          <div className="space-y-4">
            <UsageBar
              ...
              value={
                inventory.length
                  ? (dockerEnvironments.length / inventory.length) * 100
                  : 0
              }
            />
            <UsageBar
              label="Kubernetes Share"
              value={
                inventory.length
                  ? (kubernetesEnvironments.length / inventory.length) * 100
                  : 0
              }
            />
          </div>
          <div className="mt-5 space-y-3">
            <InfoTile
              label="Docker Workspaces"
              value={`${dockerEnvironments.length} environments`}
            />
            <InfoTile
              label="Kubernetes Workspaces"
              value={`${kubernetesEnvironments.length} environments`}
            />
          </div>
        </Card>
      </div>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Offline or degraded servers that should be checked first.
              </p>
            </div>
              ...
              riskServers.map((server) => (
                ...
                >
                  <div>
                    ...
                    <div className="mt-1 text-xs text-zinc-500">
                      {server.ip_address} • {server.environment ?? "unknown"}
                    </div>
                  </div>
                  <Badge
                    variant={server.status === "error" ? "error" : "warning"}
                  >
                    {server.status}
                  </Badge>
                </div>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Highest CPU pressure across Docker and Kubernetes environments.
              </p>
            </div>
            ...
            {busiestEnvironments.map((environment) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Badge variant="outline">
                    {Math.round(environment.cpuPercent ?? 0)}%
                  </Badge>
                </div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <Card className="p-6 xl:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">
                Top Docker Environments
              </h2>
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Highest-density Docker estates by containers, stacks, and
                images.
              </p>
            </div>
            <Badge variant="outline">
              {topDockerEnvironments.length} shown
            </Badge>
          </div>
          <EnvironmentRankingChart
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Real session presence from authenticated requests.
              </p>
            </div>
            ...
            {(presence?.users ?? []).slice(0, 6).map((user) => (
              ...
              >
                ...
                <div className="mt-2 text-xs text-zinc-500">
                  ...
                  windows {user.online_windows.join(", ")}
                </div>
              </div>
            ))}
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr_1.1fr]">
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-50">
                Top Kubernetes Environments
              </h2>
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Clusters ranked by nodes, pods, and namespaces.
              </p>
            </div>
            <Badge variant="outline">
              {topKubernetesEnvironments.length} shown
            </Badge>
          </div>
          <EnvironmentRankingChart
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Current estate pressure by platform and utilization class.
              </p>
            </div>
          </div>
          <div className="space-y-3">
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Latest IAM and control-plane actions for ops review.
              </p>
            </div>
...
function buildCapacityTrend(
  histories: Array<
    Array<{ cpu_usage?: number; memory_usage?: number; recorded_at?: string }>
  >,
  fallbackCpu: number,
  fallbackMemory: number,
) {
  ...
  const buckets = new Map<
    string,
    { label: string; cpuTotal: number; memoryTotal: number; count: number }
  >();
  histories.forEach((history) => {
    history.forEach((item) => {
      if (!item.recorded_at) return;
      const key = item.recorded_at.slice(0, 16);
      const label = new Date(item.recorded_at).toLocaleTimeString("en-US", {
        hour: "2-digit",
        minute: "2-digit",
      });
  ...
  return [...buckets.entries()]
    ...
    .map(([, item]) => ({
      ...
      memory: item.count
        ? Math.round(item.memoryTotal / item.count)
        : fallbackMemory,
    }));
}
...
function calculateClusterHealthScore({
  ...
  const deploymentScore =
    totalDeployments > 0 ? (healthyDeployments / totalDeployments) * 30 : 30;
...
function scoreDockerEnvironment(environment: any) {
  return (
    Number(environment.stats?.containers ?? 0) * 5 +
...
function scoreKubernetesEnvironment(environment: any) {
  return (
    Number(environment.stats?.nodes ?? 0) * 5 +
