...
import DockerTerminalWorkspaceModal, {
  type DockerTerminalWorkspaceSession,
} from "../components/DockerTerminalWorkspaceModal";
...
export default function ContainersPage() {
  ...
  const [terminalSessions, setTerminalSessions] = useState<
    DockerTerminalWorkspaceSession[]
  >([]);
  ...
  const [activeKill, setActiveKill] = useState<{
    name: string;
    id: string;
  } | null>(null);
  ...
  const { mutate: performAction, isPending: isActionPending } =
    useContainerAction(selectedServerId);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const restoredSessions = (parsed.sessions || [])
        .filter((item) => item.environmentId === selectedServerId)
        .map((item) => ({ ...item, sessionId: undefined }));
      setTerminalSessions(restoredSessions);
      setActiveTerminalTabId(
        parsed.activeTabId &&
          restoredSessions.some((item) => item.tabId === parsed.activeTabId)
          ? parsed.activeTabId
          : restoredSessions[0]?.tabId || "",
      );
    } catch {
      setTerminalSessions([]);
      setActiveTerminalTabId("");
  ...
  useEffect(() => {
    ...
    window.sessionStorage.setItem(
      `einfra.docker.terminals.${selectedServerId}`,
      JSON.stringify({
        sessions: terminalSessions.map(
          ({ sessionId: _sessionId, ...item }) => item,
        ),
  ...
  const openTerminal = (container: { id: string; name: string }) => {
    const existing = terminalSessions.find(
      (item) =>
        ...
        item.environmentId === selectedServerId,
    );
    if (existing) {
  ...
  const filteredContainers = (containers || []).filter((c) => {
    if (!c.Names[0].toLowerCase().includes(searchQuery.toLowerCase()))
      return false;
    ...
    if (activeTag) {
      const tagLabel =
        c.Labels?.[
          `einfra.tag.${activeTag.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`
        ];
  ...
  const resolveTreeGroup = (rawName: string) => {
    ...
    if (
      ...
      /^(svc|service|task|container)$/i.test(tokens[tokens.length - 1])
    ) {
  ...
  const groupedContainers = filteredContainers.reduce<
    Record<string, typeof filteredContainers>
  >((acc, container) => {
  ...
  const formatImage = (img: string) => {
    ...
    if (img.startsWith("sha256:"))
      return img.replace(/^sha256:/, "").slice(0, 12);
    return img;
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto mt-2 sm:mt-0">
          ...
          <Button
            variant="primary"
            size="md"
            onClick={() => navigate("/containers/deploy")}
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`rounded-full border px-3 py-1.5 text-xs font-medium ${activeTag === "" ? "border-blue-500 bg-blue-500 text-white" : "border-zinc-200 bg-white text-zinc-600 dark:border-zinc-800 dark:bg-[#121212] dark:text-zinc-300"}`}
        >
          All tags
        </button>
        {availableTags.map((tag) => (
          <button
            ...
            className={`rounded-full border px-3 py-1.5 text-xs font-medium ${activeTag === tag ? "border-blue-500 bg-blue-500 text-white" : "border-zinc-200 bg-white text-zinc-600 dark:border-zinc-800 dark:bg-[#121212] dark:text-zinc-300"}`}
          >
            {tag}
          </button>
        ))}
                ...
                {isLoadingContainers ? (
                  [...Array(4)].map((_, i) => (
                    <TableRow key={i}>
                      ...
                      <TableCell>
                        <div className="h-8 w-8 bg-zinc-200 dark:bg-zinc-800 rounded animate-pulse ml-auto" />
                      </TableCell>
                    </TableRow>
                  ))
                      ...
                      <TableCell>
                        <div className="flex flex-col">
                          <Link
                            ...
                            className="font-semibold text-zinc-900 dark:text-zinc-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors cursor-pointer"
                          >
                        ...
                        <div className="flex flex-col gap-0.5 max-w-[200px]">
                          {container.Ports?.filter((p) => p.PublicPort)
                            ...
                            .map((p, idx) => (
                              <span
                                key={idx}
                                className="bg-zinc-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded text-[11px] whitespace-nowrap"
                              >
                          ...
                          {container.Ports?.some((p) => !p.PublicPort) &&
                            !container.Ports?.some((p) => p.PublicPort) && (
                              <span className="text-zinc-400 text-[11px]">
                                {container.Ports.map(
                                  (p) => `${p.PrivatePort}/${p.Type || "tcp"}`,
                                )
                                  .slice(0, 2)
                                  .join(", ")}
                              </span>
                          ...
                          {container.Ports &&
                            container.Ports.filter((p) => p.PublicPort).length >
                              2 && (
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                showNotification({
                                  ...
                                  description: `Sent start signal to ${container.Names[0]}`,
                                });
                              }}
                              disabled={isActionPending}
                          ...
                          <Button
                            ...
                            onClick={() =>
                              navigate(
                                `/containers/${container.Id}/edit?envId=${selectedServerId}`,
                              )
                            }
          ...
          {Object.entries(groupedContainers).length === 0 ? (
            ...
            Object.entries(groupedContainers)
              ...
              .map(([groupName, items]) => (
                <div
                  key={groupName}
                  className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-[#121212]"
                >
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                        {items.length} container(s) sharing the same runtime
                        prefix
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      ...
                      >
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                          <div className="min-w-0">
                            <Link
                              to={`/containers/${container.Id}`}
                              className="flex items-center gap-2 font-semibold text-zinc-900 transition-colors hover:text-blue-600 dark:text-zinc-100 dark:hover:text-blue-400"
                            >
                              <ChevronRight
                                size={14}
                            ...
                            <div className="mt-1 flex min-w-0 flex-wrap gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                              ...
                              <span>•</span>
                              <span>
                                {container.Ports?.length || 0} port mapping(s)
                              </span>
                              {container.Labels?.["einfra.tags"] ? (
                                <>
                                  <span>•</span>
            ...
            performAction(
              { containerId: activeCommit.id, action: "commit", payload },
              {
                onSuccess: () => {
                  showNotification({
                    type: "success",
                    message: "Container committed",
                    description: `${activeCommit.name} -> ${payload.image}`,
                  });
                ...
                onError: (error: any) => {
                  showNotification({
                    ...
                    description:
                      error?.message || "Unable to create image snapshot.",
                  });
                },
              } as any,
            );
      ...
      {activeKill && (
        <ConfirmActionDialog
          ...
          onConfirm={() => {
            performAction({ containerId: activeKill.id, action: "kill" }, {
              onSuccess: () => {
                showNotification({
                  type: "success",
                  message: "Container killed",
                  description: activeKill.name,
                });
                setActiveKill(null);
              },
              onError: (error: any) => {
                showNotification({
                  ...
                  message: "Kill failed",
                  description:
                    error?.message || "Unable to force kill container.",
                });
              },
            } as any);
          }}
