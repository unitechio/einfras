...
export default function K8sCreateResourcePage() {
  ...
  const activeResource =
    state.activeResource || fallback?.activeResource || `${resourceType}s`;
  const description =
    state.description ||
  ...
  const defaultStarterManifest = useMemo(() => {
    if (
      typeof state.starterManifest === "string" &&
      state.starterManifest.trim()
    ) {
      return state.starterManifest;
    }
    return (
      fallback?.starterManifest(initialNamespace) ||
      "apiVersion: v1\nkind: ConfigMap\nmetadata:\n  name: sample\n"
    );
  }, [fallback, initialNamespace, state.starterManifest]);

  const initialName = useMemo(
    () => extractManifestValue(defaultStarterManifest, /^\s*name:\s*(.+)\s*$/m),
    [defaultStarterManifest],
  );
  ...
  const [templateMode, setTemplateMode] = useState<"starter" | "blank">(
    "starter",
  );
  ...
  const [resourceNamespace, setResourceNamespace] = useState(
    initialManifestNamespace,
  );
  const [labels, setLabels] = useState<Array<{ id: string; key: string; value: string; error?: string }>>([]);
  const [tagSearch, setTagSearch] = useState("");
  const [selectedTags, setSelectedTags] = useState<string[]>([]);
  const [availableTags, setAvailableTags] = useState<string[]>([]);
  const [activeTab, setActiveTab] = useState<TabKey>("form");
  const [assistedForm, setAssistedForm] = useState<AssistedFormState>(() =>
    buildAssistedFormState(defaultStarterManifest, initialNamespace),
  );
  ...
  useEffect(() => {
    if (
      ...
      selectedEnvironment.id !== selectedClusterId
    ) {
  ...
  useEffect(() => {
    ...
    if (!resourceNamespace.trim() || !namespaces.includes(resourceNamespace)) {
      setResourceNamespace(
        namespace && namespaces.includes(namespace) ? namespace : namespaces[0],
      );
    }
  ...
  const diffRows = useMemo(
    () => buildSplitDiffRows(manifest, hydratedManifest),
    [hydratedManifest, manifest],
  );
  const lastSyncedLabel = useMemo(
    () =>
      hydratedManifest === appliedRef.current ? "Synced" : "Draft changes",
    [hydratedManifest],
  );
  ...
  const visibleNamespaces = useMemo(
    () => (namespaces.length ? namespaces : ["default"]),
    [namespaces],
  );
  const applyNamespace = namespaced
    ? resourceNamespace || namespace
    : "Cluster-scoped";
  ...
  const resetEditor = (nextManifest: string, nextNamespace: string) => {
    ...
    setResourceName(
      extractManifestValue(nextManifest, /^\s*name:\s*(.+)\s*$/m),
    );
    setResourceNamespace(
      extractManifestValue(nextManifest, /^\s*namespace:\s*(.+)\s*$/m) ||
        nextNamespace,
    );
    setLabels([]);
    setSelectedTags([]);
  ...
  const handleCopyPreview = async () => {
    try {
      ...
      showNotification({
        type: "success",
        message: "YAML copied",
        description: "Preview YAML copied to clipboard.",
      });
    } catch {
      showNotification({
        type: "error",
        message: "Copy failed",
        description: "Clipboard copy is not available in this browser.",
      });
    }
  };
  ...
  return (
    <K8sExplorerLayout
      clusters={clusters}
      namespaces={
        namespaced ? visibleNamespaces : [resourceNamespace || "cluster-scoped"]
      }
      selectedCluster={selectedClusterId}
      selectedNamespace={
        namespaced ? namespace : resourceNamespace || "cluster-scoped"
      }
    ...
    >
      <div className="space-y-5 pb-16">
        <div className="rounded-md border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-[#121212]">
          <div className="grid gap-4 px-5 py-4 xl:grid-cols-[minmax(0,1fr)_560px]">
            <div className="space-y-3">
              <div>
                <h2 className="text-[2rem] font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">
                  {title}
                </h2>
                <p className="mt-1.5 max-w-3xl text-sm leading-6 text-zinc-500 dark:text-zinc-400">
                  {description}
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
                ...
                {namespaced ? (
                  <CompactStatus label="Explorer view" value={namespace} />
                ) : null}
              </div>
            ...
            >
              <label className="space-y-1.5">
                ...
                <select
                  ...
                  className="h-10 w-full rounded-md border border-zinc-200 bg-white px-3 text-sm font-medium text-zinc-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100"
                >
              ...
              <label className="space-y-1.5">
                ...
                <select
                  value={templateMode}
                  onChange={(event) =>
                    applyTemplateMode(event.target.value as "starter" | "blank")
                  }
                  className="h-10 w-full rounded-md border border-zinc-200 bg-white px-3 text-sm font-medium text-zinc-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100"
                >
              ...
              {namespaced ? (
                <label className="space-y-1.5">
                  <span className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.22em] text-zinc-500">
                    ...
                    <InfoTooltip
                      content={`This namespace is written into the manifest and used for apply. Explorer view is currently ${namespace}.`}
                    />
                  </span>
                  <select
              ...
              <label className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder="Find saved tags like prod, payments, public"
              ...
              <div className="space-y-1.5">
                <span className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.22em] text-zinc-500">
                  <span>
                    {namespaced ? "Explorer Namespace" : "Tag Matches"}
                  </span>
                  <InfoTooltip
                    content={
                      namespaced
                ...
                <div className="flex h-10 items-center rounded-md border border-zinc-200 bg-zinc-50 px-3 text-sm text-zinc-700 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200">
                  {namespaced
                    ? namespace
                    : filteredTags.length
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(380px,0.8fr)]">
          <div className="min-w-0 rounded-md border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-[#121212]">
            <div className="border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
              ...
              >
                <TabsList className="px-1">
                  <TabsTrigger value="form" icon={Wand2}>
                    Form
                  </TabsTrigger>
                  <TabsTrigger value="yaml" icon={FileCode2}>
                    YAML
                  </TabsTrigger>
                  <TabsTrigger value="diff" icon={GitCompareArrows}>
                    Diff
                  </TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="form" className="mt-5 space-y-4">
                  <CollapsibleSection
                    ...
                    subtitle="Choose metadata and basic targeting before pushing anything into YAML."
                  >
                    <div className="grid gap-3 md:grid-cols-2">
                      <FieldHint
                        label="Resource Name"
                        hint="Use a stable Kubernetes name, for example `payments-api` or `config-public`."
                      >
                        <Input
                          value={resourceName}
                          onChange={(event) =>
                      ...
                      >
                        <Input
                          value={
                            namespaced
                              ? `${applyNamespace} (from ${namespace} view)`
                              : "Cluster-scoped"
                          }
                          disabled
                        />
                      ...
                      <div className="md:col-span-2">
                        <FieldHint
                          ...
                          hint="Optional. Key-value labels to identify your resource, e.g., app.kubernetes.io/name."
                        >
                          <KeyValueEditor
                            entries={labels.length ? labels : [{ id: crypto.randomUUID?.() || Math.random().toString(), key: "", value: "" }]}
                            onChange={setLabels}
                            keyPlaceholder="app.kubernetes.io/name"
                          ...
                          <div className="mt-2 flex flex-wrap gap-2">
                            <Button
                              ...
                              variant="outline"
                              size="sm"
                              className="h-7 px-2.5 text-xs text-zinc-600 dark:text-zinc-300"
                              onClick={() => {
                                const nextLabels = [...labels.filter((l) => l.key.trim() !== ""), { id: crypto.randomUUID?.() || Math.random().toString(), key: "app.kubernetes.io/name", value: resourceName || "app" }];
                                setLabels(validateEntries(nextLabels));
                              }}
                            >
                            ...
                            <Button
                              ...
                              className="h-7 px-2.5 text-xs text-zinc-600 dark:text-zinc-300"
                              onClick={() => {
                                const nextLabels = [...labels.filter((l) => l.key.trim() !== ""), { id: crypto.randomUUID?.() || Math.random().toString(), key: "app.kubernetes.io/env", value: "production" }];
                                setLabels(validateEntries(nextLabels));
                              }}
                            >
                              Add app.kubernetes.io/env
                            </Button>
                          </div>
                        </FieldHint>
                      </div>
                  ...
                  {assistedForm.kind ? (
                    <CollapsibleSection
                      title={`${assistedForm.kind} fields`}
                      subtitle="Use structured inputs for the high-signal fields, then merge them back into YAML."
                    >
                      <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <CollapsibleSection
                    ...
                    subtitle="Selected tags are translated into Kubernetes-safe labels for downstream audit and routing."
                  >
                    <div className="flex flex-wrap gap-2">
                ...
                <TabsContent value="yaml" className="mt-5 space-y-4">
                  <div className="flex flex-wrap items-center gap-2 rounded-md border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950">
                      ...
                      <input
                        ...
                        onChange={async (event) => {
                          ...
                          try {
                            ...
                            setAssistedForm(
                              buildAssistedFormState(content, namespace),
                            );
                            showNotification({
                              type: "success",
                    ...
                    <Button
                      variant="outline"
                      onClick={() => applyTemplateMode("starter")}
                      disabled={!defaultStarterManifest.trim()}
                    >
                      Reset Template
                    </Button>
                ...
                <TabsContent value="diff" className="mt-5">
                  <div className="rounded-md border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950">
                    <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
                      <div>
                        ...
                        <div className="text-xs text-zinc-500">
                          Compare the raw YAML editor against the form-hydrated
                          output you are about to apply.
                        </div>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={applyFormToYaml}
                      >
                        Apply Diff To YAML
                      </Button>
                    </div>
                    <div className="max-h-[560px] overflow-auto p-4">
                      <div className="grid min-w-[760px] grid-cols-2 gap-3">
                        <div className="rounded-md border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-[#121212]">
              ...
              <div className="overflow-hidden rounded-md border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-[#121212]">
                <div className="flex items-center justify-between border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
                  <div>
                    ...
                    <div className="text-xs text-zinc-500">
                      This is the exact manifest payload that will be applied.
                    </div>
                  </div>
                ...
                <div className="max-h-[calc(100vh-260px)] overflow-auto p-4">
                  <YamlPreview
                    value={
                      hydratedManifest || "# YAML preview will appear here"
                    }
                    minHeight={520}
                  />
                </div>
...
function LeavePageDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-md border border-zinc-200 bg-white p-6 shadow-2xl dark:border-zinc-800 dark:bg-[#121212]">
        <div className="space-y-4">
          <div className="inline-flex h-11 w-11 items-center justify-center rounded-md border border-amber-200 bg-amber-50 text-amber-600 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-300">
          ...
          <div className="space-y-3">
            ...
            <h3 className="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">
              Leave this {resourceTitle.toLowerCase()} page?
            </h3>
            <p className="text-sm leading-6 text-zinc-500 dark:text-zinc-400">
              You still have manifest changes that have not been applied. Stay
              here to keep editing, or discard the changes and leave the page.
            </p>
          </div>
        </div>
...
function renderAssistedFields(
  ...
  setForm: Dispatch<SetStateAction<AssistedFormState>>,
) {
  switch (form.kind) {
    case "Pod":
      return (
        <>
          <FieldHint
            ...
            hint="Name of the main container inside the pod, for example `app` or `api`."
          >
            <Input
          ...
          <FieldHint
            ...
            hint="Image reference to run, for example `nginx:stable-alpine` or `ghcr.io/org/app:1.0.0`."
          >
            <Input
              value={form.image}
              onChange={(event) =>
          ...
          <FieldHint
            ...
            hint="Optional container port exposed by this pod, for example `8080`."
          >
            <Input
              value={form.port}
              onChange={(event) =>
          ...
          <FieldHint
            ...
            hint="Optional shell command used instead of the image default entrypoint."
          >
            <Input
              value={form.command}
              onChange={(event) =>
    ...
    case "Deployment":
      return (
        <>
          <FieldHint
            label="Container Name"
            hint="Name of the deployment container, usually `app` or the service name."
          >
            <Input
              value={form.containerName}
              onChange={(event) =>
          ...
          <FieldHint
            label="Container Image"
            hint="Image reference used by the deployment pods."
          >
            <Input
              value={form.image}
              onChange={(event) =>
          ...
          <FieldHint
            label="Replicas"
            hint="How many pod replicas Kubernetes should keep running."
          >
            <Input
              value={form.replicas}
              onChange={(event) =>
          ...
          <FieldHint
            label="Container Port"
            hint="Main port exposed by the application container."
          >
            <Input
              value={form.port}
              onChange={(event) =>
    ...
    case "Ingress":
      return (
        <>
          <FieldHint
            label="Host"
            hint="Public DNS name for this route, for example `api.example.com`."
          >
            <Input
              value={form.host}
              onChange={(event) =>
          ...
          <FieldHint
            label="Path"
            hint="Request path to match, usually `/` or `/api`."
          >
            <Input
              value={form.path}
              onChange={(event) =>
          ...
          <FieldHint
            ...
            hint="Existing Kubernetes Service that will receive traffic from this ingress rule."
          >
            <Input
          ...
          <FieldHint
            ...
            hint="Service port number exposed by the backend service."
          >
            <Input
              value={form.port}
              onChange={(event) =>
          ...
          <FieldHint
            label="Ingress Class"
            hint="Optional ingress controller class such as `nginx` or `traefik`."
          >
            <Input
          ...
          <FieldHint
            ...
            hint="Optional secret containing the TLS certificate for the host."
          >
            <Input
      ...
      return (
        <>
          <FieldHint
            ...
            hint="Labels used to target pods, for example `app=payments-api`."
          >
            <Input
              value={form.selector}
              onChange={(event) =>
              ...
              placeholder="app=sample-app"
            />
          </FieldHint>
          <FieldHint
            label="Service Type"
            hint="Choose how traffic reaches the service: `ClusterIP`, `NodePort`, or `LoadBalancer`."
          >
            <Input
          ...
          <FieldHint
            label="Service Port"
            hint="Port exposed by the Kubernetes Service."
          >
            <Input
              value={form.port}
              onChange={(event) =>
          ...
          <FieldHint
            label="Target Port"
            hint="Port exposed by the destination pod container."
          >
            <Input
      ...
      return (
        <div className="md:col-span-2 space-y-3">
          <FieldHint
            ...
            hint="Manage key-value pairs for your ConfigMap data. Keys must be uppercase with underscores."
          >
            <KeyValueEditor
              entries={form.configEntries.length ? form.configEntries : [{ id: crypto.randomUUID?.() || Math.random().toString(), key: "", value: "" }]}
              onChange={(nextEntries) =>
                setForm((current) => ({
                  ...current,
                  configEntries: nextEntries,
                }))
              }
              keyPlaceholder="APP_ENV"
              valuePlaceholder="production"
              requireUppercaseKey
              addButtonText="Add Config Entry"
            />
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <span className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500 mr-2">Presets</span>
              <Button
                type="button"
                variant="outline"
                size="sm"
                ...
                onClick={() => {
                  const preset = [
                    { id: crypto.randomUUID?.() || Math.random().toString(), key: "NODE_ENV", value: "production" },
                    { id: crypto.randomUUID?.() || Math.random().toString(), key: "PORT", value: "3000" },
                  ];
                  setForm((current) => ({ ...current, configEntries: validateEntries(preset, true) }));
                }}
              >
                NodeJS Env
              </Button>
              <Button
                ...
                onClick={() => {
                  const preset = [
                    { id: crypto.randomUUID?.() || Math.random().toString(), key: "SPRING_PROFILES_ACTIVE", value: "prod" },
                    { id: crypto.randomUUID?.() || Math.random().toString(), key: "SERVER_PORT", value: "8080" },
                  ];
                  setForm((current) => ({ ...current, configEntries: validateEntries(preset, true) }));
                }}
              >
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                className="ml-auto h-7 bg-indigo-50 px-2.5 text-xs font-medium text-indigo-600 hover:bg-indigo-100 dark:border-indigo-500/20 dark:bg-indigo-500/10 dark:text-indigo-300 dark:hover:bg-indigo-500/20"
                onClick={() => {
                  let normalized = form.configEntries.map(e => {
                    let nk = e.key.trim().toUpperCase().replace(/[^A-Z0-9_]/g, "_");
                    let nv = e.value.trim().replace(/^prod$/, "production").replace(/^dev$/, "development");
                    return { ...e, key: nk, value: nv };
                  });
                  setForm((current) => ({ ...current, configEntries: validateEntries(normalized, true) }));
                }}
              >
                <Wand2 className="mr-1.5 h-3 w-3" /> AI Normalize
              </Button>
            </div>
          </FieldHint>
        </div>
    ...
    case "Secret":
      return (
        <>
          <FieldHint
            label="Secret Type"
            hint="Typical values are `Opaque`, `kubernetes.io/dockerconfigjson`, or `kubernetes.io/tls`."
          >
            <Input
          ...
          <FieldHint
            ...
            hint="Enter one key/value per line in `KEY=VALUE` format. Values will be encoded into the Secret manifest."
          >
            <textarea
...
function KeyValueEditor({
  ...
  entries: Array<{ id: string; key: string; value: string; error?: string }>;
  onChange: (entries: Array<{ id: string; key: string; value: string; error?: string }>) => void;
  keyPlaceholder?: string;
  valuePlaceholder?: string;
  requireUppercaseKey?: boolean;
  addButtonText?: string;
}) {
  const updateEntry = (id: string, updates: Partial<{ key: string; value: string }>) => {
    let nextEntries = entries.map((e) => (e.id === id ? { ...e, ...updates } : e));
    nextEntries = validateEntries(nextEntries, requireUppercaseKey);
    onChange(nextEntries);
  };

  const addEntry = () => {
    let nextEntries = [...entries, { id: crypto.randomUUID?.() || Math.random().toString(), key: "", value: "" }];
    onChange(nextEntries);
  };

  const removeEntry = (id: string) => {
    if (entries.length === 1 && !entries[0].key && !entries[0].value) {
        return;
    }
    let nextEntries = entries.filter((e) => e.id !== id);
    if (nextEntries.length === 0) {
       nextEntries = [{ id: crypto.randomUUID?.() || Math.random().toString(), key: "", value: "" }];
    }
    nextEntries = validateEntries(nextEntries, requireUppercaseKey);
    onChange(nextEntries);
  };
              ...
              <div className="flex-1">
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (requireUppercaseKey) newKey = newKey.toUpperCase();
                    updateEntry(entry.id, { key: newKey });
                  }}
                  placeholder={keyPlaceholder || "Key"}
                  className={cn("font-mono text-sm", entry.error && "border-red-500 focus-visible:ring-red-500")}
                />
              </div>
...
export function validateEntries(
  ...
  return entries.map((entry) => {
    ...
    if (k) {
      ...
      } else if (requireUppercaseKey) {
        if (!/^[A-Z_][A-Z0-9_]*$/.test(k)) {
          error = "Uppercase and underscore only (e.g. APP_ENV)";
        }
      } else {
        if (!/^([a-z0-9]([-a-z0-9]*[a-z0-9])?(\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*\/)?([A-Za-z0-9][-A-Za-z0-9_.]*)?[A-Za-z0-9]$/.test(k)) {
...
function buildSplitDiffRows(beforeText: string, afterText: string) {
  ...
  return rows.length > 0
    ? rows
    : [{ type: "same" as const, before: "", after: "" }];
}
...
function YamlEditor({
  ...
  const lines = useMemo(
    () => normalizeYamlLines(value || placeholder || ""),
    [placeholder, value],
  );
      ...
      <div className="grid grid-cols-[60px_minmax(0,1fr)]">
        ...
        <div className="relative" style={{ minHeight }}>
          <div
            ref={overlayRef}
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 overflow-auto px-4 py-4 font-mono text-sm leading-6"
          >
            <YamlCode
              value={value || placeholder || ""}
              dimPlaceholder={!value.trim()}
            />
          </div>
          <textarea
            ref={textareaRef}
...
function YamlPreview({
  ...
  return (
    <div
      className="overflow-hidden rounded-md border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950"
      style={{ minHeight }}
    >
      <div className="grid grid-cols-[60px_minmax(0,1fr)]">
...
function getYamlKeyClass(key: string) {
  ...
  if (
    ["data", "stringData", "env", "ports", "containers", "volumes"].includes(
      normalized,
    )
  ) {
  ...
  if (
    ["image", "imagePullPolicy", "resources", "requests", "limits"].includes(
      normalized,
    )
  ) {
  ...
  if (
    ["type", "protocol", "serviceAccountName", "restartPolicy"].includes(
      normalized,
    )
  ) {
